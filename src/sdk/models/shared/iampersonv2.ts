...
export class IamPersonV2ComputedAttributes extends SpeakeasyBase {}

export class IamPersonV2CustomAttributes extends SpeakeasyBase {}
...
export class IamPersonV2 extends SpeakeasyBase {
    ...
    @SpeakeasyMetadata({ elemType: IamPersonV2ComputedAttributes })
    @Expose({ name: "computedAttributes" })
    @Transform(
        ({ value }) => {
            const obj: Record<string, IamPersonV2ComputedAttributes> = {};
            for (const key in value) {
                obj[key] = objectToClass(value[key], IamPersonV2ComputedAttributes);
            }
            return obj;
        },
        { toClassOnly: true }
    )
    computedAttributes?: Record<string, IamPersonV2ComputedAttributes>;
    ...
    @SpeakeasyMetadata({ elemType: IamPersonV2CustomAttributes })
    @Expose({ name: "customAttributes" })
    @Transform(
        ({ value }) => {
            const obj: Record<string, IamPersonV2CustomAttributes> = {};
            for (const key in value) {
                obj[key] = objectToClass(value[key], IamPersonV2CustomAttributes);
            }
            return obj;
        },
        { toClassOnly: true }
    )
    customAttributes?: Record<string, IamPersonV2CustomAttributes>;
